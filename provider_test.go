...
import (
	...
	"github.com/tosie/libdns-linode"
)

var (
	envToken = ""
	envZone  = ""
	// Linode will only accept certain values for TTL. Other values will be rounded to the next
	// possible value. When comparing test results with expectations, this must be taken into
	// consideration.
...
func setupTestRecords(t *testing.T, p *linode.Provider) ([]libdns.Record, testRecordsCleanup) {
...
func cleanupRecords(t *testing.T, p *linode.Provider, r []libdns.Record) {
...
func TestMain(m *testing.M) {
	envToken = os.Getenv("LIBDNS_LINODE_TEST_TOKEN")
	envZone = os.Getenv("LIBDNS_LINODE_TEST_ZONE")

	if len(envToken) == 0 || len(envZone) == 0 {
		fmt.Println(`Please notice that this test runs agains the public Linode Domains Api, so you sould
never run the test with a zone, used in production.
To run this test, you have to specify 'LIBDNS_LINODE_TEST_TOKEN' and 'LIBDNS_LINODE_TEST_ZONE'.
Example: "LIBDNS_LINODE_TEST_TOKEN="123" LIBDNS_LINODE_TEST_ZONE="my-domain.com" go test ./... -v`)
		os.Exit(1)
	}
